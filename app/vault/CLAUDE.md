...
- **Testul de rotație cu doi membri a trecut pe baza reală** (2026-09-21/22), în ordinea asta: B activat și în așteptare → fondatorul rotește (B nesigilat) → fondatorul aprobă B (primește ambele chei prin `vault_grant_deks`) → B deblochează, vede „cheia 2 · 2 chei în inel” și intrarea fondatorului → B schimbă parola master (cea veche refuzată) → B scrie sub cheia 2, fondatorul o vede → fondatorul regenerează codul → fondatorul elimină B și rotește a doua oară → B ajunge pe „Cheile lipsesc”, fără date. Ăsta e protocolul de reluat după orice schimbare la migrarea 4 sau la `lib/vault/keyring.ts`.
- Scripturile stau în scratchpad-ul sesiunii, **nu în repo**. Capcane cunoscute: `innerText` aplică `uppercase` pe eyebrow-uri (compară case-insensitive); așteaptă hidratarea înainte de submit; Enter prin CDP cere `text: "\r"`; `el.click()` nu mută focusul; `next build` strică serverul de dev din același `.next`. Din testul pe bază reală: „deblocat” se recunoaște după header-ul cu ceasul („blocare 14:58”), **nu** după „Blochează” — se potrivește case-insensitive cu „**De**blochează vault-ul” și cu „Blochează și ieși” de pe ecranul de așteptare; lista și fâșia de fapte se încarcă **după** header, iar `keyringStatus` după titlul panoului — așteaptă-le explicit înainte de captură; titlul unei intrări apare în editor înainte de salvare — „salvat” înseamnă contorul crescut și butonul fără „Se salvează…”; `document.body` poate fi `null` în timpul unei navigări.
