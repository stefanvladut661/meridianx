...
### CE MAI E DE FĂCUT — starea la 2026-09-22 (citește întâi asta)

Codul e COMPLET (11 faze). **Vault-ul e verificat pe Supabase-ul real și livrat în `main`** (2026-09-22): migrările 3 + 4 aplicate, sign-up oprit, fondator activat, cod pe hârtie, iar **testul de rotație cu doi membri a trecut integral** (protocolul e în `app/vault/CLAUDE.md` §5). Ce a rămas:

1. **Migrarea 5 — SCRISĂ (2026-09-25), DE APLICAT de om** (`supabase/migrations/00000000000005_admin_emails.sql`): în SQL editor, apoi IMEDIAT `insert into public.admin_emails (email) values ('…');` cu aceleași adrese ca în `ADMIN_EMAILS`. Portalul de reclame (migrarea 6, `feat/ads-portal`) depinde de ea. Descrierea inițială: tabelă `admin_emails (email text primary key)` cu RLS și fără politici (o citește doar o funcție `security definer` `is_lead_admin()` care compară `lower(auth.jwt() ->> 'email')`); politicile `to authenticated using (true)` de pe `leads` / `lead_events` se înlocuiesc cu `using (public.is_lead_admin())`. **Atenție:** panoul de admin CITEȘTE prin sesiune (`createReadOnlySessionClient`, deci prin RLS) și scrie prin service role — după migrare, fără rânduri în `admin_emails`, panoul vede zero lead-uri (nu crapă). Migrarea trebuie să se încheie cu un `do $$ … raise warning` dacă tabela e goală, iar omul inserează aceleași adrese ca în `ADMIN_EMAILS` imediat după. Membrii de vault (adrese separate, decizia 1 din faza 2) nu mai văd lead-urile. Fiecare `update`/`delete` din ea cu `where` explicit (`safeupdate`, vezi mai jos).
2. **Observație UX, de decis:** omul nu a găsit ștergerea unei intrări. Există, dar e un link estompat în josul fișei, sub istoric („Șterge intrarea", lângă „Duplică intrarea"), cu confirmare inline și coș. Prea discretă pentru o acțiune pe care o cauți; candidat: un rând de acțiuni vizibil în capul fișei sau în meniul rândului din listă. Nu s-a schimbat nimic încă.
3. **Cunoscut, acceptat:** scenariile de regresie (seed criptat + backend fals) s-au pierdut cu scratchpad-ul; tiparul e în `app/vault/CLAUDE.md` §5. Driverul membrului de test (`vault-b.mjs` + `b.json`) e în memoria asistentului, în afara repo-ului; contul de test B a fost șters din Authentication → Users la finalul testului.

**Cum s-a încheiat testul de rotație (2026-09-21 → 22).** Bug găsit și reparat pe bază reală: `vault_rotate_dek` a răspuns „UPDATE requires a WHERE clause" — Supabase rulează extensia `safeupdate` pe rolul API-ului, care refuză UPDATE/DELETE fără WHERE chiar și în funcții `security definer` și chiar pe tabele cu un singur rând. Cele două `update public.vault_meta` din migrarea 4 (`vault_rotate_dek`, `vault_rewrap_recovery`) au primit `where id;` (`id` e `boolean primary key check (id)`); omul a re-aplicat funcțiile în SQL editor, **una pe fișier, un fișier pe tab, fără selecție** — cu ambele într-un singur bloc, editorul rulase doar statementul de sub cursor, de două ori. Apoi, în ordine, cu asistentul ca membrul B (Chrome headless + CDP pe dev local legat la baza reală) și omul ca fondator: B activat și în așteptare → fondatorul a rotit cheia cu B nesigilat → l-a aprobat pe B, care a primit **ambele** chei (`vault_grant_deks`, nicio notă de „cheie lipsă") → B a deblocat: „cheia 2 · rotită de fondator · 2 chei în inel · toate rândurile pe cheia curentă", plus intrarea „Test fondator" decriptată → B și-a schimbat parola master (`vault_rekey_self` + `vault_reseal_self`); parola veche: „Email sau parolă master greșite", cea nouă deblochează aceleași date → B a scris „Test rotație 204721" sub cheia 2 (2 intrări · 2 clienți la re-deblocare; fondatorul a văzut-o după reîncărcare) → fondatorul a regenerat codul (`vault_rewrap_recovery`) → l-a eliminat pe B și a rotit a doua oară → B, cu parola lui validă, ajunge pe „Sesiune deschisă. Cheile lipsesc." — sesiune Supabase, zero date, singurul drum fiind o cerere nouă aprobată de un membru activ. Consola curată pe fiecare pas. Deblocarea durează 5–13 s pe laptopul de dezvoltare (Argon2id + rețea).
