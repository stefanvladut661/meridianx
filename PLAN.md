...
### Faza 3 — terminat (listă + căutare)

Tot în `app/vault/` și `lib/vault/`; niciun fișier partajat atins, nicio dependență nouă.

- **`lib/vault/entries.ts` — CONTRACTUL PAYLOAD-ULUI, lege pentru fazele 4–6.** Intrare: `{ v: 1, kind, title, fields[], tags[], notes }`, unde `kind ∈ login | server | api | database | card | note | other`, iar fiecare câmp e `{ label, value, secret, kind: text | url | multiline }`. Client: `{ v: 1, name }`. `loadVault(supabase, dek)` descarcă toate rândurile vii (`deleted_at is null`) și le decriptează local, într-o singură trecere (XChaCha20 deschide sute de rânduri în milisecunde; serverul n-are ce indexa). Un rând care nu se decriptează sau n-are forma contractului devine `status: "unreadable"` — vizibil în listă, cu roșu, nu ascuns: un rând alterat pe server e informație pentru om. `parseEntryPayload` e defensiv (câmpuri lipsă → valori goale, `kind` necunoscut → `other`), ca o schemă viitoare `v: 2` să poată coexista.
- **`lib/vault/search.ts`** — pură. Fără diacritice și majuscule (NFD + strip), tokeni legați cu ȘI, `#etichetă` restrânge la etichete. **Secretele nu intră în index** — altfel numărul de rezultate ar fi un canal prin care se ghicește o parolă literă cu literă. Rezultatele rămân grupate pe client (așa gândește agenția: „parolele de la X"), potrivirea pe titlu urcă rândul în grup.
- **UI:** antetul are comutatorul Intrări / Membri, cu numărul cererilor în așteptare pe „Membri" (fondatorul trebuie să le vadă fără să deschidă vederea); pe telefon comutatorul coboară pe rândul al doilea. `useVaultData` (`use-vault-data.ts`) e singura sursă pentru ambele vederi (intrări + membri + meta), cu reîncărcare manuală și la revenirea în filă după 30 s; `MembersPanel` primește datele prin props. Lista: grupuri pe client, rânduri-buton cu titlu + primul câmp nesecret + etichete; **tipul apare doar când nu e „login"** — regula nu se etichetează, excepțiile da. Fișa: coloană lipicioasă pe ≥1024, dialog cu focus captiv sub; secretele mascate cu **lungime fixă** (12 puncte — lungimea reală e informație), „Arată" cu ascundere automată la 30 s și contor la vedere, „Copiază" fără descoperire. Fâșia de fapte de sub titlu — „N intrări · M clienți · decriptate local în X ms" — e afirmația modelului zero-knowledge, cu cifre reale.
- **`clipboard.ts`** — ce se copiază se golește după 45 s, necondiționat (modelul Bitwarden; fără `readText`, care cere permisiune în Chrome și lipsește în Firefox); dacă fila nu e în prim-plan, golirea se reia la focus. La blocare, shell-ul se demontează și golește clipboard-ul dacă e ceva de golit.
- **Tastatură:** `/` deschide căutarea de oriunde, `↓` din căutare intră în listă, `↑↓ Home End` umblă prin rânduri (peste capetele de grup), `Enter` deschide, `Esc` închide fișa / golește căutarea; la închidere focusul se întoarce pe rândul intrării. Coloana fișei goală nu rămâne goală: listează exact aceste scurtături.
- **Verificat** în Chrome headless (CDP) cu backend Supabase fals și un seed criptat în Node cu `lib/vault/crypto.ts` (DEK real, AD real, un rând mutat pe alt id → ilizibil, un rând șters): 37 de verificări trecute — deblocare, fapte, grupare, rândul șters lipsește, rândul alterat marcat, niciun secret în listă; căutare (`ads`, `#plati` fără diacritice, secret → „Nimic pentru", ȘI între tokeni); fișă (mascat, Arată + contor, Copiază → clipboard + notă, schimbarea intrării re-maschează); tastatură (Enter/Escape, `/`, săgeți, Home/End, Tab cu focus vizibil); vederea Membri cu cererea în așteptare; 360 fără scroll orizontal, dialog cu focus captiv și revenire pe rând; reduced-motion la 768; blocare → clipboard gol. Consolă curată. Build de producție verde.

### Decizii luate în faza 3 (confirmate de om, 2026-09-20)

1. **Totul se decriptează la deblocare, în memorie.** Nu există paginare sau decriptare la cerere: vault-ul unei agenții are sute de intrări, nu zeci de mii, iar căutarea instant peste tot conținutul valorează mai mult decât memoria economisită. Dacă vreodată depășește ~5.000 de rânduri, se revine aici.
2. **Câmpurile sunt o listă liberă, nu un formular fix per tip.** `kind` decide doar câmpurile PROPUSE la adăugare (faza 4) și eticheta din listă; payload-ul acceptă orice câmpuri, în orice ordine — ordinea salvată e ordinea afișată. Așa importul (faza 5) poate păstra ce are fișierul sursă fără să forțeze totul în „utilizator + parolă".
3. **Golirea clipboard-ului e necondiționată** după 45 s (nu se verifică dacă mai e valoarea noastră) — vezi `clipboard.ts`. UI-ul spune asta în clar când copiezi un secret.
4. **Fără TOTP** în faza asta: ar cere HMAC-SHA1, pe care libsodium nu-l are (intenționat); WebCrypto îl are. Se poate adăuga ca `kind: "totp"` de câmp într-o fază ulterioară, fără schimbare de schemă.

### Faza 4 — terminat (adăugare + editare)

Tot în `app/vault/` și `lib/vault/`; niciun fișier partajat atins, nicio dependență nouă, `crypto.ts` neatins.

- **`lib/vault/entries.ts` — scrieri.** `createClient` / `renameClient` / `deleteClient` (soft), `createEntry` / `updateEntry` / `deleteEntry` (soft). **Id-ul se generează în browser** (`crypto.randomUUID`) înainte de criptare, ca datele adiționale (`vault_entries:<id>`) să lege ciphertextul de rând încă de la insert. **Concurență optimistă:** update-ul și ștergerea cer `version` = cea văzută (`.eq("version")` + `.select("id")`); zero rânduri → `conflict` cu mesaj („Altcineva a modificat intrarea între timp"). Versiunea o crește triggerul, nu clientul. **Ștergerea ultimei intrări vii a unui client șterge și clientul** — clienții se creează doar la salvarea unei intrări, deci nu există niciodată clienți goi. `templateFields(kind)` = câmpurile PROPUSE per tip; `normalizeTags` = tăiate, fără `#`, fără duplicate (case-insensitive), cu majusculele cum au fost tastate.
- **`lib/vault/generate.ts`** — generator de parole pe **WebCrypto** (`getRandomValues` + eșantionare prin respingere, alegere uniformă), nu pe libsodium: regula „libsodium doar din `crypto.ts`" rămâne întreagă. Litere mereu, cifre și simboluri opționale, 8–64 caractere; `entropyBits` = `length · log2(alfabet)`, afișată ca atare.
- **UI.** „Adaugă intrare" în antet, „+ adaugă" și „redenumește" pe capul fiecărui grup (mereu vizibile, mono estompat — pe telefon nu există hover), „Adaugă prima intrare" în starea goală. `EntryEditor` în aceeași ramă ca fișa (antet / corp derulabil / subsol lipit cu „Salvează"): titlu, tip, client (existent sau „+ Client nou", creat abia la salvare), câmpuri libere cu etichetă, valoare, „secret", fel (text / URL / text lung), ↑ ↓ Elimină, „+ Câmp" / „+ Câmp secret"; tipul rescrie șablonul DOAR cât câmpurile sunt neatinse. Secretele în editor sunt `type=password` cu „Arată" și `autocomplete=new-password` (ca browserul să nu propună parola master). Generatorul e bloc inline sub câmp, cu entropia în mono. Etichete prin virgulă; notițe. `Ctrl/⌘ + Enter` salvează. Fișa are „Editează" și „Șterge intrarea" (confirmare inline care spune și dacă dispare clientul). **Gardă la modificări nesalvate:** Escape, „Anulează", clic pe alt rând sau pe alt „adaugă" peste un editor murdar întreabă întâi („Renunță la modificări" / „Continuă editarea") și ține minte unde voia să ajungă omul. Escape închide acum și coloana de pe desktop, nu doar dialogul. După salvare, rândul intră în ecran; după ștergere, focusul merge în căutare.
- **Verificat** în Chrome headless cu backend-ul fals extins (POST/PATCH cu semantica PostgREST: `Prefer: return=representation`, versiunea crește la schimbarea payload-ului) — **53 de verificări** pentru faza 4 + **37** regresie faza 3: șablon per tip și rescrierea lui doar cât e neatins; client nou + generator (24 caractere, entropie afișată, valoarea intră în câmp și se arată); salvare cu Ctrl+Enter → 17 intrări · 6 clienți, fișa cu parola mascată, etichete deduplicate, v1; **în backend payload-ul e hex fără text în clar, clientul nou e criptat și legat**; editare cu conflict simulat (versiune schimbată pe server → mesaj) apoi salvare → v2; garda de modificări nesalvate în toate cele trei căi; „+ adaugă" pre-completează clientul; ștergerea ultimei intrări șterge și clientul (16 · 5, soft delete pe ambele rânduri); redenumire; 360: editor în dialog cu focus pe titlu, fără scroll orizontal, Escape peste modificări întreabă; vault gol → „Adaugă prima intrare" cu „+ Client nou" preselectat. Consolă curată, build de producție verde.

### Decizii luate în faza 4 (confirmate de om, 2026-09-20 — „alege tu varianta mai bună")

1. **Ștergerea e soft și versionată.** `deleted_at` se pune cu aceeași verificare de `version` ca update-ul: o intrare rescrisă de altcineva între timp nu se șterge orbește. Istoricul rămâne pe server (faza 6 îl arată; tot atunci poate veni și „coșul" pentru restaurare).
2. **Clienții nu au versiune** — redenumirea e „ultimul câștigă". Numele e un singur câmp, conflictul real e improbabil și consecința e minoră.
3. **Etichetele păstrează majusculele tastate**, dedup case-insensitive; căutarea le normalizează oricum.
4. **Fără drag-and-drop la câmpuri** — ↑ ↓ ajung, merg de la tastatură și n-au nevoie de o librărie.

### Faza 5 — terminat (import + duplicare)

Tot în `app/vault/` și `lib/vault/`; niciun fișier partajat atins, nicio dependență nouă.

- **`lib/vault/import.ts` — pur.** Parser CSV RFC 4180 propriu (ghilimele, ghilimele duble, rânduri noi în celule, CRLF, BOM-ul de la Excel, separator `,` / `;` / tab detectat din antet). **Rolurile coloanelor** (titlu, URL, utilizator, parolă, notițe, client/folder, TOTP) detectate după sinonime — acoperă exporturile Chrome/Edge, Bitwarden, 1Password, LastPass, KeePass și antete românești; o coloană are un singur rol; coloanele administrative (`favorite`, `reprompt`, `type`, `fields`…) se ignoră, **restul coloanelor necunoscute devin câmpuri cu numele lor** — nu se pierde nimic. `planImport` întoarce intrări gata de criptat, cu **duplicatele marcate** (client + titlu + utilizator, normalizate; față de vault ȘI față de rândurile de dinainte din fișier) și rândurile goale; titlul lipsă vine din host-ul URL-ului; rând fără parolă/URL/utilizator → `note`. 20 de verificări în Node (Chrome cu virgule și rânduri noi în notiță, Bitwarden cu foldere și TOTP, Excel RO cu `;` și BOM, LastPass, duplicat intern).
- **`lib/vault/entries.ts`**: `createEntries` (loturi de 50, fiecare intrare cu id și nonce propriu, progres raportat; la eroare, ce a intrat rămâne — reluarea le vede ca duplicate) și `ensureClients` (clienții existenți se refolosesc după nume normalizat, cei lipsă se creează).
- **UI (`import-panel.tsx`)** în aceeași ramă ca fișa, trei pași unul sub altul: fișier (`<input type=file>`, citit local) sau text lipit → revizuire (ce coloană e ce, cu selecturi; **în ce client ajung**: „Din coloana „folder"" cu lista clienților din fișier și fallback-ul vizibil „Import fără folder", sau „Toate într-un singur client" existent/nou; bilanț „N intrări noi · M duplicate (sărite) · K rânduri goale"; bifă „Importă și duplicatele"; previzualizare cu primele 8 rânduri, **fără parole nici mascate**) → import cu contor real „Se importă… 23 din 140" → bilanț („3 intrări importate · 2 clienți noi") + îndemnul de a șterge CSV-ul, care e în clar.
- **Duplică** — din fișă, sub câmpuri, lângă „Șterge": editorul pornește cu o copie („(copie)"), același client, toate câmpurile; se salvează ca intrare nouă. Editorul primește `template`; eyebrow „Copie · client".
- **Verificat** în Chrome headless (48 de verificări): import Bitwarden lipit → roluri detectate corect, clientul din coloană propus, 3 noi + 1 duplicat, „Importă 3 intrări" → 19 · 7, **backend cu payload-uri hex fără text în clar**, clienți noi criptați; al doilea import → 4 duplicate, buton dezactivat la 0, bifa duplicatelor → „Importă 4"; un singur client + suprascrierea rolurilor (titlu lipsă → host); **fișier real prin `DOM.setFileInputFiles`** (Chrome CSV cu virgulă în notiță) → client nou → 20 · 8, notița întreagă în fișă; duplicare cPanel → copie cu parola în câmp secret, același client, salvată → 21; 360: importul în dialog, fără scroll orizontal. Consolă curată, build de producție verde.

### Decizii luate în faza 5 (alese de asistent, la cererea omului)

1. **CSV, nu JSON-urile native.** Toate managerele exportă CSV; JSON-ul Bitwarden ar aduce câmpuri personalizate, dar și un al doilea parser și un al doilea drum de testat. Dacă vreodată e nevoie, se adaugă lângă, cu același `planImport`.
2. **Duplicatele se sar implicit** și se pot importa la cerere; cheia e client + titlu + utilizator. Un import rulat de două ori nu dublează vault-ul.
3. **Folderul devine client**, cu fallback vizibil pentru rândurile fără folder — nu se inventează clienți din URL.
...
### Fazele 9–11 — terminat (export criptat · rotația cheii · import JSON)

`crypto.ts` neatins în toate trei. **Migrarea 4** (`00000000000004_vault_keyring.sql`) e aditivă și OBLIGATORIE de acum (clientul citește `dek_id`).

**Faza 10 — rotația cheii de date (inelul de chei).** Modelul, în migrarea 4: cheia originală rămâne (istoricul e criptat cu ea și nu se poate rescrie), rotația adaugă o cheie nouă în `vault_deks`, sigilată pentru fiecare membru activ în `vault_member_deks` (serverul refuză dacă lipsește vreunul), împachetată cu un **cod de recuperare nou** (cel vechi moare — altfel un ex-membru care l-a văzut ar deschide tot); toate cheile mai vechi se re-împachetează cu noul cod. Fiecare rând poartă `dek_id` (`null` = originala). Triggerul: `dek_id` schimbat = re-criptare, fără versiune nouă. Funcții noi: `vault_rotate_dek` (atomic), `vault_grant_deks`, `vault_reseal_self`, `vault_rewrap_recovery` (regenerarea codului trece de acum pe aici). Client: `KeyMaterial` = `{ dek (curentă), currentDekId, ring: Map, publicKey, privateKey }`; citirea ia cheia rândului din inel (`dekFor`), scrierea folosește mereu cheia curentă și o notează; `openKeys` deschide legacy + toate sigilările; aprobarea acordă și cheile rotite; recuperarea cu codul și schimbarea parolei re-sigilează inelul; **membrul fără cheia curentă e oprit la deblocare** (mesaj: cere-o) — mai bine decât să scrie sub cheia retrasă. Re-criptarea (`reencryptAll`) e per rând, cu verificare de versiune, reluabilă, cu contor; rândurile ilizibile se sar și se numără. Panoul „Cheia de date" (Membri): cheia curentă (a câta, de când, de cine), câte rânduri sunt încă pe chei vechi + „Reia re-criptarea", cui îi lipsesc chei + „Acordă cheile", „Rotește cheia" cu confirmare → codul nou (o singură dată) → re-criptare cu contor. După eliminarea unui membru activ, panoul de membri sugerează rotația. Editarea unui rând aflat pe cheie veche îl aduce întâi pe cea curentă (două update-uri), ca istoricul să nu se piardă.

**Faza 9 — export criptat** (`lib/vault/export.ts`, `export-panel.tsx`, „Exportă" în antet). JSON cu antet în clar (format, versiune, câte intrări, parametri KDF, salt) și un singur blob: clienți + intrări (payload-uri decriptate) sub o **parolă de export** separată, Argon2id la parametrii vault-ului + XChaCha20-Poly1305, salt aleator per fișier (dat lui `deriveMasterMaterial` ca „email" `export:<hex>` — fără să atingem `crypto.ts`). Doar rândurile vii și lizibile; fără istoric și coș. Descărcare din browser, nimic prin server. Minimum 12 caractere, nerecuperabilă.

**Faza 11 — import JSON** (`classifyJson` în `import.ts`). Bitwarden necriptat: foldere/colecții → client, login (mai multe URL-uri, utilizator, parolă, TOTP → cod 2FA), card (titular, brand, număr/CVV secrete, expirare), notă securizată, identitate (CNP/pașaport/permis secrete), câmpuri personalizate (hidden → secret, boolean → da/nu, linked sărit). Bitwarden criptat → refuz cu explicație. Fișierul propriu de export → pasul cu parola de export (inelul la vedere), apoi aceeași revizuire (clientul din export, duplicate). `planImport` a fost împărțit în „rânduri → candidați" + `finishPlan` (duplicatele), folosit de toate sursele.

**Verificat** (72 de verificări într-un singur scenariu, backend fals cu migrarea 4 — tabele, `dek_id`, versionarea cu re-criptare, cele patru funcții, `or=(…)`): rotația → cod nou cu 8 grupuri, cheie nouă activă, sigilată pentru ana, `rotations = 1`; re-criptare: 21 de rânduri (inclusiv coșul), 1 sărit (rândul alterat din seed, care nu se poate decripta — rămâne pe cheia veche și se vede „1 rând încă pe chei vechi"), istoricul neatins (2 versiuni, `dek_id null`), zero versiuni noi; după rotație: lista decriptată, istoricul lizibil, editarea → v4 pe cheia nouă cu istoric pe cheia nouă; blocare → deblocare cu inelul din bază; regenerarea codului (rotations 2, prin `vault_rewrap_recovery`); **recuperarea cu codul nou după rotație** → ambele chei, totul lizibil, cheia rotită re-sigilată; export: validare, derivare cu inelul, fișier cu antet corect (argon2id, 256 MiB, salt aleator), **nimic în clar în fișier**, descărcat cu numele datei; import înapoi: recunoscut fără parolă, parola greșită refuzată, cu parola bună → 15 duplicate (tot vault-ul e deja aici); Bitwarden JSON: 3 intrări din 1 folder + fallback, câmpuri ascunse mascate, text în clar, boolean „da", al doilea URL, TOTP viu, cardul cu număr/CVV mascate; exportul Bitwarden criptat refuzat; 360 fără scroll orizontal. Regresie fazele 3–8: 37+53+48+39+29+32 verificări (o singură bătaie de clipboard în headless). Build de producție verde.

### Decizii luate în fazele 9–11 (alese de asistent)

1. **Rotația generează cod de recuperare nou** — singura variantă în care un ex-membru care a văzut codul vechi nu deschide cheile de după el. Se notează pe hârtie, ca la inițializare.
2. **Cheile vechi nu se șterg niciodată**; istoricul rămâne pe ele. Garanția rotației e „nimic scris DUPĂ nu se deschide cu cheile de dinainte" — nu mai mult, și e scris în migrare și în UI.
3. **Re-criptarea e în browser, per rând, reluabilă** — nu într-o funcție SQL: serverul n-are cheile, deci nici n-ar putea. O rotație întreruptă lasă rânduri sub ambele chei, toate lizibile.
4. **Migrarea 4 e obligatorie**, nu opțională cu dublu drum în client: două căi de cod pentru același lucru sunt două locuri de greșit. Mesajul de eroare spune exact ce lipsește.
...
### CE MAI E DE FĂCUT — starea la 2026-09-21, seara (citește întâi asta)

Codul e COMPLET (11 faze) pe `feat/vault`. **Prima deblocare reală a mers** (omul, pe Supabase-ul real, migrările 3 + 4 aplicate, sign-up oprit, fondator activat, cod pe hârtie). Suntem în mijlocul **testului de rotație cu doi membri**, oprit la un bug de SQL. Ce urmează, în ordine:

1. **Om — aplică patch-ul pentru `safeupdate` și confirmă.** Pe bază reală, `vault_rotate_dek` a răspuns „UPDATE requires a WHERE clause": Supabase rulează extensia `safeupdate` pe rolul API-ului, care refuză UPDATE/DELETE fără WHERE chiar și în funcții `security definer`. Cele două `update public.vault_meta … ;` din migrarea 4 (în `vault_rotate_dek` și `vault_rewrap_recovery`) n-aveau WHERE; tranzacția s-a derulat înapoi, baza e curată. **Migrarea 4 din repo e corectată** (`where id;`), dar baza are încă funcțiile vechi: omul trebuie să ruleze în SQL editor cele două blocuri `create or replace function` din `supabase/migrations/00000000000004_vault_keyring.sql` (liniile `vault_rotate_dek` … `$$;` și `vault_rewrap_recovery` … `$$;`), **pe rând, într-un tab nou, fără selecție** — două încercări cu tot blocul odată au lăsat funcțiile neschimbate (probabil editorul a rulat doar selecția / statementul de sub cursor). Verificare: `select proname, prosrc like '%where id;%' as patched from pg_proc where proname in ('vault_rotate_dek','vault_rewrap_recovery');` → ambele `true`. Dacă apare „must be owner of function", funcțiile sunt ale altui rol decât cel din editor.
2. **Testul de rotație — protocol mixt** (asistentul = membrul B din Chrome headless pe dev local legat la Supabase-ul real; omul = fondatorul în browserul lui). Stare: **pasul 1 e făcut** — contul de test B (`driveagency001+vaultb@gmail.com`, creat de om cu parolă temporară) a fost activat de asistent, cheile depuse prin `vault_join`, B e **în așteptare, neaprobat**. Driverul (`vault-b.mjs`, comenzi `activate | unlock | status | change-password | add-entry`) și configul lui (`b.json`, cu parolele master de aruncat ale lui B) sunt salvate în memoria asistentului, în afara repo-ului. `.env.local` local are acum URL-ul și cheia anon reale. Pașii rămași:
   - 2 (om): Membri → Cheia de date → **Rotește cheia** cât B e încă în așteptare → notează codul nou → re-criptează. Atinge `vault_rotate_dek` + triggerul `dek_id`.
   - 3 (om): **Aprobă** B → `vault_approve_member` + `vault_grant_deks` (singurul drum natural prin grant: aprobarea de după rotație).
   - 4 (asistent): `node vault-b.mjs unlock` + `status` — B decriptează intrările, panoul arată cheia #2.
   - 5 (asistent): `change-password` → `vault_rekey_self` + `vault_reseal_self`, apoi blocare / deblocare cu parola nouă (`master2`).
   - 6 (asistent + om): `add-entry master2` — intrare scrisă sub cheia curentă; omul o vede după reîncărcare.
   - 7 (om): **Regenerează codul** → `vault_rewrap_recovery`; codul nou e cel care rămâne.
   - 8 (om, apoi asistent): **Elimină** B → panoul sugerează rotația → a doua rotație (re-împachetează cheile vechi); asistentul verifică că B nu mai intră. La final, omul șterge contul B din Authentication → Users.
3. **Asistent — după test:** `PLAN.md` + `app/vault/CLAUDE.md` la zi, commit, **merge `feat/vault` → `main`**, push (Vercel face deploy din `main`). Nu înainte de pasul 2.
4. **Asistent — migrarea 5** (`00000000000005_admin_emails.sql`, aditivă, pe branch propriu după merge): tabelă `admin_emails (email text primary key)` cu RLS și fără politici (o citește doar o funcție `security definer` `is_lead_admin()` care compară `lower(auth.jwt() ->> 'email')`); politicile `to authenticated using (true)` de pe `leads` / `lead_events` se înlocuiesc cu `using (public.is_lead_admin())`. **Atenție:** panoul de admin CITEȘTE prin sesiune (`createReadOnlySessionClient`, deci prin RLS) și scrie prin service role — după migrare, fără rânduri în `admin_emails`, panoul vede zero lead-uri (nu crapă). Migrarea trebuie să se încheie cu un `do $$ … raise warning` dacă tabela e goală, iar omul inserează aceleași adrese ca în `ADMIN_EMAILS` imediat după. Membrii de vault (adrese separate, decizia 1 din faza 2) nu mai văd lead-urile.
5. **Cunoscut, acceptat:** scenariile de regresie (seed criptat + backend fals) s-au pierdut cu scratchpad-ul; tiparul e în `app/vault/CLAUDE.md` §5.

### De făcut de către om (înainte de a folosi vault-ul)

- [x] (2026-09-21) Aplică migrarea 3 ȘI migrarea 4 în SQL editor, în ordine (nu există CLI/Docker local; `db push` ar reîncerca și migrările 1–2, aplicate manual). Fără migrarea 4, deblocarea se oprește cu „Inelul de chei lipsește… aplică migrarea 4".
- [x] (2026-09-21) Supabase → Authentication → Providers → Email: **oprește „Allow new users to sign up"**; membrii se invită de acolo.
- [ ] Aceeași bază ca lead-urile — vezi observația despre politicile `leads` din tabel.
- [x] (2026-09-21) Creează primul cont de vault (Authentication → Users → Add user → „Create new user", parolă temporară, auto-confirm), **pe altă adresă decât cea din `ADMIN_EMAILS`** (decizia 1 din faza 2). Apoi `/vault` → „Prima intrare sau parolă pierdută? Activează contul". Primul cont activat devine fondator și primește codul de recuperare — **pe hârtie**.
- [x] (2026-09-21, pentru testul de rotație) Local, dacă vrei să rulezi `/vault` pe calculatorul tău: `NEXT_PUBLIC_SUPABASE_URL` și `NEXT_PUBLIC_SUPABASE_ANON_KEY` în `.env.local` (pe Vercel există deja, pentru admin).
